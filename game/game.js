...
const NIGHT_PARAMS = {
  ...
  3: {
    ...
    clockWindPerClick:  1,
    kaaFirstMove:       [24, 42],
    kaaMoveInterval:    [6, 10],
    kaaDoorWait:        [6, 10],
    ukkichiFirstMove:     [30, 48],
    ukkichiMoveInterval:  [7, 12],
    ukkichiDoorWait:      [6, 10],
    potamaruFirstMusic:   [30, 48],
...
function buildCustomNightParams() {
  ...
  // レベル→移動間隔（秒）: lv0=不動, lv1=18s, lv20=4s
  function toInterval(l) { return l === 0 ? [9999,9999] : (function(){ var v=18-(l-1)*(14/19); return [Math.max(4,v-2),Math.max(4,v+2)]; })(); }
  // レベル→ドア待機（秒）: lv0=不動, lv1=10s, lv20=3s
  function toDoorWait(l) { return l === 0 ? [9999,9999] : (function(){ var v=10-(l-1)*(7/19); return [Math.max(3,v-1),Math.max(3,v+1)]; })(); }
  return {
    powerBaseDrain: 0.06,
    clockDrain: lv.kitsune === 0 ? 0 : 0.4 + (lv.kitsune / 20) * 3.0,
    clockWindPerClick: 1,
